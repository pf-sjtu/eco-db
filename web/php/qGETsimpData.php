<?php
isset($_GET['table_name']) && isset($_GET['dtBegStr']) && isset($_GET['dtEndStr']) && isset($_GET['num']) || die('{"phpErrorCode": 0, "phpError": "Parameters error."}');
...
$qLineTotal = "SELECT (@total:=COUNT(*)) FROM ".$_GET['table_name']." WHERE datetime >= '".$_GET['dtBegStr']."' AND datetime <= '".$_GET['dtEndStr']."';";
$setParams  = "SET @id = 0, @num = ".$_GET['num'].";";
$qSimpData  = "SELECT * FROM ".$_GET['table_name']." WHERE datetime >= '".$_GET['dtBegStr']."' AND datetime <= '".$_GET['dtEndStr']."' AND (@tmp:=(@id:=@id+1)*@num/@total)-FLOOR(@tmp)<@num/@total;";
