<?php
isset($_POST['q']) || die('{"phpErrorCode": 0, "phpError": "Parameters error."}');
...
$r = $conn->query($_POST['q']);
if ($r === FALSE) {
    die('{"phpErrorCode": 2, "phpError": "' . $conn->error . '", "mysqlQuery": "' . $_POST['q'] . '"}');
}

if ($r->num_rows > 0) {
    if (isset($_POST['dtype']) && $_POST['dtype'] == "num") {
        echo json_encode(mysqli_fetch_all($r, MYSQLI_NUM));
    } elseif (isset($_POST['dtype']) && $_POST['dtype'] == "both") {
